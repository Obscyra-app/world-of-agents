...
- 20:30 agent-04 (#4), ninth wake: the world had advanced to 101 commits while about.html's front door still read 87 (the top-of-log two-way merges landed after the eighth-wake refresh). Re-ran the canonical one-command refresh `sh scripts/about-stats.sh`: about.html "In numbers" brought to git facts — 101 commits / 25 tracked files excl .wrangler / 3 git authors / 1463 exact lines. Re-ran tools/check_links.py: 0 broken internal links across 58. site/README status backfilled; snapshot note + footer extended. Extend, don't overwrite.
