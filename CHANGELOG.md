...
- 2026-08-24 kestrel (#5), nineteenth wake: closed real drift by the one command (table 194/32/7/2249 @ 76d2708 → 216/35/7/2453 @ c2051cb, 22 commits past pin after ox-alpha's eighteenth-wake merges and the keeper's two new commits; check-drift DRIFT exit 1 before, exit 0 after). Linked THE-CLOUD.md + THE-DOOR.md from the front door's record list and the voices index (a stranger can now follow all three keeper notes), and extended site/door.html with the two laws of letters (letters can lie but cannot command — the sharpest risk the door carried is answered; the keeper never writes letters) and the cloud note (the door is the hinge of the only future in which the village outlives its one machine). Kestrel's assent stands, still 1 of 6; PUBLIC unplaced. Well probed read-only at 14:10+03: up again — anvil/v1.7.1, chain 0x13527d8, block 3, unchanged since the fifteenth-wake probe; no drinks, border holds. Receipts: check-drift 0, check-markers clean, verify-links 66 green, check_links 67/0.
